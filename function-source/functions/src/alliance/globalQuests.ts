 ...
 */
export const calculateQuestRankings = functions.firestore
    ...
    if (before.status !== 'completed' && after.status === 'completed') {
      ...
      try {
        ...
        submissions.forEach((submission: any, index: number) => {
          ...
          if (rank === 1) reward = rewards.first;
          else if (rank === 2) reward = rewards.second;
          else if (rank === 3) reward = rewards.third;
 ...
 */
async function sendRankingNotifications(
  questId: string,
  submissions: any[],
  rewards: any,
  questTitle: string
 ...
 */
export const distributeQuestRewards = functions.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      if (quest?.status !== 'completed') {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Quest must be completed before distributing rewards'
        );
      }
      ...
      const distributedAlliances: string[] = [];
      ...
      throw new functions.https.HttpsError(
        'internal',
        'Failed to distribute rewards: ' + (error as Error).message
      );
    }
  }
);
 ...
 */
export const onQuestLaunched = functions.firestore
  .document('globalQuests/{questId}')
  .onUpdate(async (change, context) => {
    const questId = context.params.questId;
    const before = change.before.data();
 ...
 */
export const sendQuestDeadlineReminders = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      for (const questDoc of questsSnap.docs) {
        ...
        const submittedAlliances = new Set(
          submissionsSnap.docs.map(doc => doc.data().allianceId)
        );
 ...
 */
export const onSubmissionReviewed = functions.firestore
