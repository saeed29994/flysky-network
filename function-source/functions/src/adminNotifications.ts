...
// Interface for gift distribution data
interface GiftDistributionData {
  title: string;
  message: string;
  amount: number;
  reason: string;
...
// Interface for welcome bonus data
 ...
 */
export const distributeGifts = functions
  .runWith({
    timeoutSeconds: 300,
    memory: '512MB'
  })
  .https.onCall(async (data: GiftDistributionData, context) => {
    // Authentication check
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'User must be authenticated to distribute gifts'
      );
    }
    ...
    try {
      ...
      if (!callerData || callerData.role !== 'admin') {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Only administrators can distribute gifts'
        );
      }
      ...
      if (!title || !message || !reason || amount <= 0) {
        throw new functions.https.HttpsError(
          'invalid-argument',
          'Invalid gift data: title, message, reason are required and amount must be positive'
        );
      }
      ...
      switch (targetType) {
        ...
        case 'multiple':
          if (!userIds || userIds.length === 0) {
            throw new functions.https.HttpsError(
              'invalid-argument',
              'User IDs are required for single/multiple target types'
            );
          }
        ...
        case 'plan':
          if (!planName) {
            throw new functions.https.HttpsError(
              'invalid-argument',
              'Plan name is required for plan target type'
            );
          }
      ...
      if (targetUserIds.length === 0) {
        throw new functions.https.HttpsError(
          'not-found',
          'No users found for the specified target criteria'
        );
      }
 ...
 */
export const grantWelcomeBonus = functions
  .runWith({
    timeoutSeconds: 60,
    memory: '256MB'
  })
  .https.onCall(async (data: WelcomeBonusData, context) => {
 ...
 */
export const sendInternationalizedAdminNotification = functions
  .runWith({
    timeoutSeconds: 60,
    memory: '256MB'
  })
