    ...
    if (isNewUser) {
      ...
      if (finalReferral) {
        try {
          ...
          // Continue with sign in even if referral fails, but log the error
          // Don't block user sign in due to referral limit
        }
      }

      // Send welcome message using secure cloud function
      try {
        await GiftService.grantWelcomeBonus(user.uid, 100, 'Google Sign-In welcome bonus');
      } catch (error) {
        console.error('Failed to grant Google welcome bonus:', error);
    ...
    try {
      ...
      if (!inboxSnap.exists()) {
        await setDoc(inboxRef, {
          ...
          body: 'Welcome! You received 100 FSN as a welcome bonus!',
          timestamp: Date.now(),
          read: false,
          claimed: false,
          amount: 100,
