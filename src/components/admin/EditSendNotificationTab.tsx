...
import { useNotifications, NotificationPayload, Notification } from '../../hooks/useNotifications';
...
interface EditSendNotificationTabProps {
  notification: Notification;
  onClose: () => void;
  onSuccess: () => void;
}

const EditSendNotificationTab = ({ notification, onClose, onSuccess }: EditSendNotificationTabProps) => {
  const [title, setTitle] = useState(notification.title);
  const [body, setBody] = useState(notification.message);
  const [targetAudience, setTargetAudience] = useState<'all' | 'premium' | 'new' | 'inactive' | 'custom' | 'plans'>('all');
  const [selectedPlatforms, setSelectedPlatforms] = useState<string[]>(notification.platforms || ['mobile', 'web', 'inbox']);
    ...
    <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-white/20 shadow-xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">{t('admin.notifications.modals.editNotification')}</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="md:col-span-2 flex flex-col sm:flex-row gap-3 pt-4">
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 px-4 py-2 sm:py-3 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl transition-colors text-sm sm:text-base flex items-center justify-center gap-2"
            >
              {loading ? (
              ...
              {loading ? t('admin.notifications.loading.sending') : (scheduleDate ? t('admin.notifications.actions.schedule') : t('admin.notifications.actions.resend'))}
            </button>
          </div>
        </div>
      </div>
...
export default EditSendNotificationTab;