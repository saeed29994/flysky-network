...
const MiningCardBackup = ({ plan, onClaim, balance }: MiningCardProps) => {
  ...
  const [currentPlanData, setCurrentPlanData] = useState<FirebasePlan | null>(
    null
  );
  ...
  // Since users claim every 12 hours, we use half the daily limit for the cycle
  const miningRate =
    dailyLimit > 0 && MINING_CYCLE_SECONDS > 0
      ? dailyLimit / 2 / MINING_CYCLE_SECONDS
      : 0;
  const halfDailyLimit = dailyLimit / 2; // Maximum mining per 12-hour cycle
  ...
  const fetchUserData = async () => {
    ...
    if (snap.exists()) {
      ...
      if (!startTimestamp) {
        ...
        const startDate =
          typeof startTimestamp.toDate === "function"
  ...
  useEffect(() => {
    if (userData && initialized) {
      const currentMined =
        typeof contextDailyMined === "number" && !isNaN(contextDailyMined)
          ? contextDailyMined
          : 0;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const currentMined = Math.min(
        halfDailyLimit,
        isNaN(rawMined) ? 0 : rawMined
      );

      setMined(currentMined);

      if (
        (elapsed >= MINING_CYCLE_SECONDS || currentMined >= halfDailyLimit) &&
        !claimReady
      ) {
        setClaimReady(true);
        setIsMaxed(true);
        ...
        const checkAndSendNotification = async () => {
          ...
          try {
            ...
            const hasRecentNotification = snapshot.docs.some((doc) => {
              ...
              return (
                data.type === "claim_reward" &&
                data.title?.includes("Mining Complete") &&
                data.timestamp?.toDate() > new Date(Date.now() - 5 * 60 * 1000)
              ); // Last 5 minutes
            });
            ...
            if (!hasRecentNotification && !sentNotification.current) {
              ...
              notifyMiningComplete()
                .then(() => {
                  console.log(
                    "✅ Mining completion notification triggered successfully"
                  );
                  sentNotification.current = true;
                })
                .catch((error) => {
                  console.error(
                    "❌ Error triggering mining notification:",
                    error
                  );
                });
            } else {
            ...
            console.error(
              "❌ Error checking for existing notifications:",
              error
            );
            ...
            if (!sentNotification.current) {
              notifyMiningComplete()
                .then(() => {
                  console.log(
                    "✅ Mining completion notification triggered successfully (fallback)"
                  );
                  sentNotification.current = true;
                })
                .catch((error) => {
                  console.error(
                    "❌ Error triggering mining notification (fallback):",
                    error
                  );
                });
            }
      ...
      <div className="w-full min-h-[calc(100vh-4rem)] bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex justify-center items-center px-4">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-white mb-4">
            {t("miningPage.loading")}
          </h2>
          <p className="text-gray-400 mb-6">
            {t("miningPage.loadingPlanData")}
          </p>
        </motion.div>
      </div>
    );
    ...
    return (
      <div className="w-full min-h-[calc(100vh-4rem)] bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex justify-center items-center px-4">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-white mb-4">
            {t("miningPage.planNotFound")}
          </h2>
          <p className="text-gray-400 mb-6">
            {t("miningPage.planNotFoundDesc")}
          </p>
        </motion.div>
      </div>
    );
  ...
  if (firstTime) {
    return (
      <div className="w-full min-h-[calc(100vh-4rem)] bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex justify-center items-center px-4">
        ...
        >
          <div
            className={`w-16 h-16 bg-gradient-to-r ${currentPlanData.color} rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg`}
          >
            <FaCoins className="w-8 h-8 text-white" />
          </div>
          <h2 className="text-2xl font-bold text-white mb-4">
            {t("miningPage.readyToMine")}
          </h2>
  ...
  return (
    <div className="w-full bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 pb-12">
        {/* Banner Section - Right after main mining card - Simplified */}
        <div className="bg-white/5 rounded-xl p-3 mb-4 mt-4 border border-white/10 shadow-lg relative">
          <div className="relative max-w-2xl mx-auto text-center">
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-3 leading-tight">
              💎🚀 Boost Your Mining Power!
            </h2>
            <p className="text-base md:text-lg text-white/80 mb-2">
              Start earning every 12 hours — each FSN you mine is already valued
              at $0.01
            </p>
            <p className="text-sm md:text-base text-yellow-300 font-semibold">
              The Public Sale will be announced soon — stay tuned and keep
              mining!
            </p>
          </div>
        </div>
          ...
          <div className="xl:col-span-8 space-y-8">
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white">
                      {t("miningPage.balance")}
                    </h3>
                    <p className="text-xs text-gray-400">
                      {t("miningPage.available")}
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20 shadow-xl">
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`w-10 h-10 bg-gradient-to-r ${currentPlanData.color} rounded-xl flex items-center justify-center shadow-lg`}
                  >
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20 shadow-xl">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white">
                      {t("miningPage.progress")}
                    </h3>
                    <p className="text-xs text-gray-400">
                      {t("miningPage.currentCycle")}
                    </p>
                  </div>
                </div>
                <div className="text-center">
                  ...
                  <div className="text-xs text-yellow-400 font-semibold">
                    {Math.round((mined / halfDailyLimit) * 100)}%{" "}
                    {t("miningPage.complete")}
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="text-center mb-6">
                ...
                <div className="text-xl text-yellow-400 font-semibold mb-4">
                  {t("miningPage.fsn")} {t("mining")}
                </div>
                ...
                <div className="text-sm text-gray-400">
                  {Math.floor(mined)} / {dailyLimit / 2} {t("miningPage.fsn")}
                </div>
              </div>
            ...
            >
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-xl">
                <div className="flex items-center gap-4 mb-6">
                  <div
                    className={`w-14 h-14 bg-gradient-to-r ${currentPlanData.color} rounded-2xl flex items-center justify-center shadow-lg`}
                  >
                    <FaCoins className="w-7 h-7 text-white" />
                  </div>
                  <div>
                    <h2 className="text-xl font-bold text-white">
                      {t("miningPage.fsnDailyMining")}
                    </h2>
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-white">
                      {t("miningPage.miningStats")}
                    </h3>
                    <p className="text-sm text-gray-400">
                      {t("miningPage.performanceOverview")}
                    </p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-gray-400 text-sm">
                      {t("miningPage.currentMined")}
                    </span>
                    <span className="text-yellow-400 font-semibold">
                      {Math.floor(mined)} FSN
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-400 text-sm">
        ...
        {/* Earnings Table - At the bottom, full width */}
        <motion.div
        ...
        >
          <Card className="bg-white/10 backdrop-blur-sm border-white/20 shadow-2xl w-full">
            <CardContent className="p-8">
              <h2 className="text-3xl font-bold text-white text-center mb-8">
                Expected Mining Earnings (at $0.01/FSN)
              </h2>
                ...
                <table className="w-full text-white">
                  <thead>
                    <tr className="border-b border-white/20">
                      ...
                      <th className="text-center py-4 px-6 font-bold text-lg">
                        Earnings per 12h
                      </th>
                      <th className="text-center py-4 px-6 font-bold text-lg">
                        Earnings per Month (~60 cycles)
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-b border-white/10 hover:bg-white/5 transition-colors">
                      <td className="py-6 px-6 font-semibold text-lg">
                        Economy
                      </td>
                       <td className="text-center py-6 px-6 text-purple-400 font-bold text-lg">
                        10 FSN = $0.10
                      </td>
                       <td className="text-center py-6 px-6 text-purple-400 font-bold text-lg">
                        600 FSN = $6.00
                    ...
                    <tr className="border-b border-white/10 hover:bg-white/5 transition-colors">
                      ...
                      <td className="text-center py-6 px-6 text-blue-400 font-bold text-lg">
                        50 FSN = $0.50
                      </td>
                      <td className="text-center py-6 px-6 text-blue-400 font-bold text-lg">
                        3,000 FSN = $30.00
                    ...
                    <tr className="hover:bg-white/5 transition-colors">
                      <td className="py-6 px-6 font-semibold text-lg">
                        ...
                        <td className="text-center py-6 px-6 text-green-400 font-bold text-lg">
                        100 FSN = $1.00
                      </td>
                       <td className="text-center py-6 px-6 text-green-400 font-bold text-lg">
                        6,000 FSN = $60.00
...
export default MiningCardBackup;
