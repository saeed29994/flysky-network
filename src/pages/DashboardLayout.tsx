...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      const unsubscribeSnapshot = onSnapshot(userRef, (userSnap) => {
        try {
          ...
          if (data?.block) {
            // console.log('DashboardLayout: User is blocked, redirecting to user-blocked');
      ...
      // Only create inbox listener if user is still authenticated
      ...
      if (user) {
        ...
        unsubscribeInbox = onSnapshot(inboxQuery, (snapshot) => {
          ...
          // Don't set error state for permission errors during logout
        });
      }
      
      // Store the listener only if it was created
      if (unsubscribeInbox) {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Sign out after cleaning up
      await signOut(auth);
      navigate('/login');
    } catch (error) {
      console.error('❌ Logout failed:', error);
      // Still try to sign out even if token deletion fails
      try {
        await signOut(auth);
        navigate('/login');
      } catch (signOutError) {
