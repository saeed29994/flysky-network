...
const Settings = () => {
  ...
  // Sign-in methods states
  const [signInMethods, setSignInMethods] = useState<string[]>([]);
  const [showAddPassword, setShowAddPassword] = useState(false);
  const [newSignInPassword, setNewSignInPassword] = useState('');
  const [confirmNewSignInPassword, setConfirmNewSignInPassword] = useState('');
  ...
  const handleAddPassword = async () => {
    if (!newSignInPassword || !confirmNewSignInPassword) {
      toast.error(t('settingsSection.fillAllFields'));
      return;
    }
    ...
    try {
      setLoadingSignInMethods(true);
      const credential = EmailAuthProvider.credential(user.email, newSignInPassword);
      await linkWithCredential(user, credential);

      toast.success('Password added successfully! You can now sign in with your email and password.');
      setShowAddPassword(false);
      setNewSignInPassword('');
      setConfirmNewSignInPassword('');
  ...
  const handleRemoveSignInMethod = async (method: string) => {
    const user = auth.currentUser;
    if (!user) return;
    ...
    try {
      ...
      toast.error(error.message || 'Failed to remove sign-in method');
    } finally {
      setLoadingSignInMethods(false);
    }
  };
  ...
  const tabs = [
    ...
    { id: 'signInMethods', label: 'Sign-In Methods', icon: Key },
    ...
    switch (activeTab) {
      ...
      case 'signInMethods':
        return (
          <div className="space-y-6">
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              <h3 className="text-lg font-semibold text-white mb-4">Sign-In Methods</h3>
              <p className="text-gray-300 text-sm mb-6">
                Manage how you sign in to your account. You can add multiple sign-in methods for easier access.
              </p>
              ...
              {loadingSignInMethods ? (
                ...
                <div className="space-y-4">
                  ...
                  {signInMethods.map((method) => (
                    <div key={method} className="flex items-center justify-between p-4 bg-white/5 rounded-lg">
                        ...
                        <div>
                          <h4 className="text-white font-medium capitalize">
                            {method === 'password' ? 'Email & Password' :
                             ...
                             method === 'phone' ? 'Phone Number' : method}
                          </h4>
                          <p className="text-sm text-gray-400">
                            {method === 'password' ? 'Sign in with your email and password' :
                             method === 'google.com' ? 'Sign in with your Google account' :
                      ...
                      {signInMethods.length > 1 && (
                        <button
                          onClick={() => handleRemoveSignInMethod(method)}
                          className="text-red-400 hover:text-red-300 p-2 rounded-lg hover:bg-red-500/10 transition-colors"
                          title={`Remove ${method} sign-in method`}
                        >
                    ...
                    <div className="border-2 border-dashed border-white/20 rounded-lg p-4">
                      {!showAddPassword ? (
                        <div className="text-center">
                          ...
                          <p className="text-gray-300 mb-4">Add email and password sign-in</p>
                          <button
                            onClick={() => setShowAddPassword(true)}
                            className="bg-gradient-to-r from-blue-500 to-purple-500 text-white px-4 py-2 rounded-lg font-medium hover:from-blue-600 hover:to-purple-600 transition-all duration-200 flex items-center gap-2 mx-auto"
                          >
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <h4 className="text-white font-medium">Add Password Sign-In</h4>
                            <button
                          ...
                          <div>
                            ...
                            <input
                              type="password"
                              value={newSignInPassword}
                              onChange={(e) => setNewSignInPassword(e.target.value)}
                              className="w-full px-3 py-2 border border-white/20 rounded-lg bg-white/10 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          ...
                          <div>
                            ...
                            <input
                              type="password"
                              value={confirmNewSignInPassword}
                              onChange={(e) => setConfirmNewSignInPassword(e.target.value)}
                              className="w-full px-3 py-2 border border-white/20 rounded-lg bg-white/10 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          ...
                          <button
                            ...
                            disabled={!newSignInPassword || !confirmNewSignInPassword || loadingSignInMethods}
                            className="w-full bg-gradient-to-r from-green-500 to-blue-500 text-white py-2 rounded-lg font-medium hover:from-green-600 hover:to-blue-600 transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {loadingSignInMethods ? (
                  ...
                  <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="text-blue-400 font-medium mb-1">Security Note</h4>
                        <p className="text-sm text-gray-300">
                          Adding multiple sign-in methods makes your account more secure and provides backup access options.
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-white/10">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between py-3">
                  ...
                  <CustomSelect
                    value={language}
                    onChange={(value) => handleLanguageChange({ target: { value } } as any)}
                    options={[
