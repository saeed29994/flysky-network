  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen w-full max-w-3xl mx-auto text-white">
        <h1 className="text-2xl font-bold mb-4 text-yellow-400">📬 {t('inbox')}</h1>

        <div className="flex space-x-4 mb-6">
          <button
            onClick={() => setActiveTab('inbox')}
            className={`px-4 py-2 rounded ${activeTab === 'inbox' ? 'bg-yellow-500 text-black' : 'bg-gray-800 text-white'}`}
          >
            📥 {t('menu.inbox')}
          </button>
          <button
            onClick={() => setActiveTab('archived')}
            className={`px-4 py-2 rounded ${activeTab === 'archived' ? 'bg-yellow-500 text-black' : 'bg-gray-300 text-black'}`}
          >
            📂 {t('archived')}
          </button>
          <button
            onClick={() => setActiveTab('trash')}
            className={`px-4 py-2 rounded ${activeTab === 'trash' ? 'bg-yellow-500 text-black' : 'bg-gray-300 text-black'}`}
          >
            🗑️ {t('trash')}
          </button>
        </div>
        ...
        {messages.length === 0 ? (
          <p className="text-gray-400">{t('referralBonus.noBonus')}</p>
        ) : (
            ...
            <div className="bg-gray-900 p-6 rounded-lg shadow-xl w-full max-w-md">
              ...
              <div className="flex justify-between">
                {!selectedMessage.archived && !selectedMessage.deleted && (
                  <>
                    <button onClick={() => handleArchive(selectedMessage)} className="text-blue-400 hover:text-blue-300">
                      📥 {t('archive')}
                    </button>
                    <button onClick={() => handleDelete(selectedMessage)} className="text-red-400 hover:text-red-300">
                      🗑️ {t('delete')}
                    </button>
                  </>
                )}
