...
export const deleteCurrentToken = async (uid: string) => {
  try {
    ...
    // Only try to get/delete token if permission is granted
    if (Notification.permission === 'granted') {
      try {
        ...
        // Ignore token errors during logout - user might have blocked permissions
        console.warn('⚠️ Could not delete FCM token during logout:', (tokenError as Error).message);
      }
    }
    ...
    if (uid) {
      try {
        ...
        // console.log('🗑️ تم حذف التوكن من قاعدة البيانات');
      } catch (tokenError) {
