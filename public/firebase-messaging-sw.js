...
self.addEventListener('message', (event) => {
  console.log('[Firebase Service Worker] Received message from app:', event.data);
  if (event.data && event.data.type === 'AUTH_USER') {
    currentUserId = event.data.userId;
    console.log('[Firebase Service Worker] Auth user set:', currentUserId);
  }
});
...
messaging.onBackgroundMessage(function(payload) {
  ...
  self.registration.showNotification(title, notificationOptions)
    .then(() => console.log('[Firebase Service Worker] Notification shown'))
